...
// ---------------------------------------------------------------------------
// VAL-FIREWORKS-018: Fireworks logs and responses remain secret-safe.
//
...
// fwCredentialSentinels are unique synthetic values placed in credential-bearing
...
// in captured logs.
var fwCredentialSentinels = []string{
	"fw_envlog_secret_xyz",       // provider credential (FIREWORKS_API_KEY)
	"fw_querylog_secret_abcde",   // query-string credential (?api_key=)
	"fw_bodylog_secret_fghij",    // request-body credential field ("apiKey":)
	"fw_clientlog_token_uvwxy",   // downstream client auth header
	"fw_resplog_token_klmno",     // credential-named field in response body ("token":)
	"fw_errlog_credential_pqrst", // credential-named field in error body ("api_key":)
}

func assertNoFWLogSentinels(t *testing.T, logs string) {
	t.Helper()
	for _, s := range fwCredentialSentinels {
...
// newFWLogServer builds a real server with a Fireworks model pointing at the
// given upstream handler. The logger writes to the returned buffer. If trace
// is true, trace-request logging is enabled.
func newFWLogServer(t *testing.T, upstreamHandler http.HandlerFunc, trace bool) (*Server, *bytes.Buffer) {
	t.Helper()
...
`+traceYAML+`models:
  - alias: fw-log-test
    ...
    upstream_model: accounts/fireworks/models/log-test
...
// fwRespBodyWithCredField is a success response that contains ordinary content
...
const fwRespBodyWithCredField = `{"id":"x","choices":[{"index":0,"message":{"role":"assistant","content":"ordinary response text"}}],"token":"fw_resplog_token_klmno"}`

// TestFireworksLogSafety_DefaultLogging_NoCredentialLeak exercises default
...
func TestFireworksLogSafety_DefaultLogging_NoCredentialLeak(t *testing.T) {
	t.Setenv("FIREWORKS_API_KEY", "fw_envlog_secret_xyz")

	s, logs := newFWLogServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(fwRespBodyWithCredField))
	}, false)
	...
	cases := []struct {
	...
	}{
		{
			...
			body: `{"model":"fw-log-test","messages":[{"role":"user","content":"hi"}]}`,
		},
		{
			name: "tool_call",
			path: "/v1/chat/completions",
			body: `{"model":"fw-log-test","messages":[{"role":"user","content":"call tool"}],"tools":[{"type":"function","function":{"name":"f","parameters":{"type":"object"}}}]}`,
		},
		{
			name: "query_credential",
			path: "/v1/chat/completions?api_key=fw_querylog_secret_abcde",
			body: `{"model":"fw-log-test","apiKey":"fw_bodylog_secret_fghij","messages":[]}`,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req.Header.Set("Authorization", "Bearer fw_clientlog_token_uvwxy")
			s.Engine().ServeHTTP(w, req)

			// No credential sentinel may appear in default logs.
			assertNoFWLogSentinels(t, logs.String())
			...
			if strings.Contains(logs.String(), "ordinary response text") {
...
// TestFireworksLogSafety_TraceLogging_NoCredentialLeak exercises bounded
...
func TestFireworksLogSafety_TraceLogging_NoCredentialLeak(t *testing.T) {
	t.Setenv("FIREWORKS_API_KEY", "fw_envlog_secret_xyz")
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// "token":"fw_resplog_token_klmno" should become "token":"***".
			if strings.Contains(logs.String(), "fw_resplog_token_klmno") {
...
// TestFireworksLogSafety_TraceStreaming_NoCredentialLeak verifies that trace
...
func TestFireworksLogSafety_TraceStreaming_NoCredentialLeak(t *testing.T) {
	t.Setenv("FIREWORKS_API_KEY", "fw_envlog_secret_xyz")

	sseFrames := []string{
		`data: {"id":"1","choices":[{"index":0,"delta":{"content":"streaming content"}}]}`,
		`data: {"id":"1","token":"fw_resplog_token_klmno"}`,
		`data: [DONE]`,
	}
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions",
		strings.NewReader(`{"model":"fw-log-test","stream":true,"messages":[{"role":"user","content":"hi"}]}`))
	req.Header.Set("Authorization", "Bearer fw_clientlog_token_uvwxy")
	s.Engine().ServeHTTP(w, req)
	...
	if !strings.Contains(logs.String(), "streaming content") {
		t.Errorf("trace stream log missing ordinary content:\n%s", logs.String())
	}
}

// TestFireworksLogSafety_UpstreamError_NoCredentialLeak verifies that an
...
func TestFireworksLogSafety_UpstreamError_NoCredentialLeak(t *testing.T) {
	t.Setenv("FIREWORKS_API_KEY", "fw_envlog_secret_xyz")

	errBody := `{"error":{"message":"rate limited","type":"rate_limit_error"},"api_key":"fw_errlog_credential_pqrst"}`
	...
	for _, trace := range []bool{false, true} {
		...
		t.Run(label, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions",
				strings.NewReader(`{"model":"fw-log-test","messages":[]}`))
			req.Header.Set("Authorization", "Bearer fw_clientlog_token_uvwxy")
			s.Engine().ServeHTTP(w, req)
...
// TestFireworksLogSafety_MissingKeyFailsLocally verifies that missing profile
// key produces a local error with zero upstream requests and no credential
// leakage in logs.
func TestFireworksLogSafety_MissingKeyFailsLocally(t *testing.T) {
	// Do NOT set FIREWORKS_API_KEY.
	...
	if upstreamCalled {
		t.Fatal("upstream must not be called when FIREWORKS_API_KEY is missing")
	}
