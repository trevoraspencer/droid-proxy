...
func TestWorkflowValidation_RuntimeSmokeBinaryStartReadinessShutdownCleanup(t *testing.T) {
	preflightPortFree(t, "127.0.0.1:8787")
	...
	for time.Now().Before(deadline) {
		resp, err := http.Get("http://127.0.0.1:8787/health")
		if err == nil {
	...
	for _, path := range []string{"/healthz", "/v1/models", "/models"} {
		resp, err := http.Get("http://127.0.0.1:8787" + path)
		if err != nil {
	...
	waitForPortFree(t, "127.0.0.1:8787", 2*time.Second)

	invalidConfig := filepath.Join(t.TempDir(), "invalid-nonloopback.yaml")
	if err := os.WriteFile(invalidConfig, []byte(strings.Replace(workflowValidationConfigYAML(fake.URL(), 8787), "host: 127.0.0.1", "host: 0.0.0.0", 1)), 0o600); err != nil {
		t.Fatal(err)
	}
...
func preflightPortFree(t *testing.T, addr string) {
	t.Helper()
	conn, err := net.DialTimeout("tcp", addr, 100*time.Millisecond)
	if err == nil {
		_ = conn.Close()
		t.Fatalf("runtime smoke fixed port %s is already in use", addr)
	}
}
